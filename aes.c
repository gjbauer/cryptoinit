// The code in this file was originally written by Sam Trenholme
// and was released to the public domain. The original files 
// are available at:
// http://www.samiam.org/rijndael.html
// http://www.samiam.org/key-schedule.html
// http://www.samiam.org/galois.html
// and http://www.samiam.org/s-box.html
//
// Modifications made by Jesse Kornblum and are also released
// to the public domain. Summary of modifications:
// * Modified key schedule code to verify a key schedule,
//   not just compute them.

#include <string.h>
#include <inttypes.h>
#include <stdlib.h>

#include "aes.h"

// Rotate a 32-bit word
static void rotate(unsigned char *in) 
{
  unsigned char a,c;
  a = in[0];
  for(c = 0; c < 3; c++) 
    in[c] = in[c + 1];
  in[3] = a;
  return;
}


static unsigned char gmul(unsigned char a, unsigned char b) 
{
  unsigned char p = 0;
  unsigned char counter;
  unsigned char hi_bit_set;

  for(counter = 0; counter < 8; counter++) 
  {
    if((b & 1) == 1) 
      p ^= a;
    hi_bit_set = (a & 0x80);
    a <<= 1;
    if(hi_bit_set == 0x80) 
      a ^= 0x1b;		
    b >>= 1;
  }
  return p;
}

static unsigned char rcon(unsigned char in) 
{
  unsigned char c=1;

  if(in == 0)  
    return 0; 
  while(in != 1) {
    c = gmul(c,2);
    in--;
  }
  return c;
}


// Log table using 0xe5 (229) as the generator 
static unsigned char ltable[256] = {
   };

// Anti-log table:
static unsigned char atable[256] = {
   };


static unsigned char gmul_inverse(unsigned char in) 
{
  // 0 is self inverting 
  if(in == 0) 
    return 0;
  else
    return atable[(255 - ltable[in])];
}


static unsigned char sbox(unsigned char in) 
{
  unsigned char c, s, x;
  s = x = gmul_inverse(in);
  for(c = 0; c < 4; c++) 
  {
    // One bit circular rotate to the left 
    s = (s << 1) | (s >> 7);
    // xor with x 
    x ^= s;
  }
  x ^= 99; // 0x63 
  return x;
}


// This is the core key expansion, which, given a 4-byte value,
// does some scrambling
static void schedule_core(unsigned char *in, unsigned char i) 
{
  // Switched from char to int to avoid compiler warnings.
  uint8_t a;

  // Rotate the input 8 bits to the left 
  rotate(in);
  // Apply Rijndael's s-box on all 4 bytes 

  for(a = 0; a < 4; a++) 
    in[a] = sbox(in[a]);

  // On just the first byte, add 2^i to the byte 
  in[0] ^= rcon(i);
}

int hamming_distance(const unsigned char * first_buffer, const unsigned char * second_buffer, size_t size_in_bytes)
{
    int distance=0;
    for (size_t i=0; i < 8*size_in_bytes; i++)
    {
        if ( (i % 8) == 0 && i > 0 )
        {
            first_buffer++;
            second_buffer++;
        }
        int f = (*first_buffer & (1 << (i % 8))) >> (i % 8);
        int s = (*second_buffer & (1 << (i % 8))) >> (i % 8);
        if (f != s) distance++;
    }
    return distance;
}

// Returns TRUE if the buffer computed contains a valid AES-128 key
// schedule, otherwise, FALSE.
int valid_aes128_schedule(const unsigned char * in, unsigned char computed[AES128_KEY_SCHEDULE_SIZE])
{
  unsigned char t[4];

  // c is 16 because the first sub-key is the user-supplied key
  unsigned char pos = AES128_KEY_SIZE;
  unsigned char i = 1;
  unsigned char a;

  if (NULL == in)
    return FALSE;
  
  //memcpy(computed, in, AES128_KEY_SIZE);

  // We need 11 sets of sixteen bytes each for 128-bit mode
  while (pos < AES128_KEY_SCHEDULE_SIZE) 
  {
    // Copy the temporary variable over from the last 4-byte block
    memcpy (t, computed + pos - 4, 4);

    // Every four blocks (of four bytes), do a complex calculation 
    if (pos % AES128_KEY_SIZE == 0) 
    {
      schedule_core(t,i);
      i++;
    }

    for (a = 0; a < 4; a++) 
    {
      computed[pos] = computed[pos - AES128_KEY_SIZE] ^ t[a];

      pos++;
    }
  }

  // If the computed schedule doesn't match our goal,
  // punt immediately!
  if (hamming_distance(in, computed, AES128_KEY_SCHEDULE_SIZE) > 8)
    return FALSE;

  return TRUE;
}


// Returns TRUE if the buffer computed contains a valid AES-192 key
// schedule, otherwise, FALSE.
int valid_aes192_schedule(const unsigned char * in, unsigned char computed[AES192_KEY_SCHEDULE_SIZE])
{
  unsigned char t[4];

  // c is 24 because the first sub-key is the user-supplied key
  unsigned char pos = AES192_KEY_SIZE;
  unsigned char i = 0;
  unsigned char a;

  if (NULL == in)
    return FALSE;
  
  //memcpy(computed, in, AES192_KEY_SIZE);

  // We need 11 sets of sixteen bytes each for 128-bit mode
  while (pos < AES192_KEY_SCHEDULE_SIZE) 
  {
    // Copy the temporary variable over from the last 4-byte block
    memcpy (t, computed + pos - 4, 4);

    // Every six blocks (of four bytes), do a complex calculation 
    if (pos % AES192_KEY_SIZE == 0) 
    {
      schedule_core(t,i);
      i++;
    }

    for (a = 0; a < 4; a++) 
    {
      computed[pos] = computed[pos - AES192_KEY_SIZE] ^ t[a];

      pos++;
    }
  }

  // If the computed schedule doesn't match our goal,
  // punt immediately!
  if (hamming_distance(in, computed, AES192_KEY_SCHEDULE_SIZE) > 8)
    return FALSE;

  return TRUE;
}



// Returns TRUE if the buffer computed contains a valid AES-256 key
// schedule, otherwise, FALSE.
int valid_aes256_schedule(const unsigned char * in, unsigned char computed[AES256_KEY_SCHEDULE_SIZE])
{
  unsigned char t[4];

  // c is 32 because the first sub-key is the user-supplied key
  unsigned char pos = AES256_KEY_SIZE;
  unsigned char i = 1;
  unsigned char a;

  if (NULL == in)
    return FALSE;
  
  memcpy(computed, in, AES256_KEY_SIZE);

  // We need 11 sets of sixteen bytes each for 256-bit mode
  while (pos < AES256_KEY_SCHEDULE_SIZE) 
  {
    // Copy the temporary variable over from the last 4-byte block
    memcpy (t, computed + pos - 4, 4);

    // Every eight blocks (of four bytes), do a complex calculation 
    if (pos % AES256_KEY_SIZE == 0) 
    {
      schedule_core(t,i);
      i++;
    }

    // For 256-bit keys, we add an extra sbox to the calculation
    if (16 == pos % AES256_KEY_SIZE)
    {
      for (a = 0 ; a < 4 ; ++a)
	    t[a] = sbox(t[a]);
    }

    for (a = 0; a < 4; a++) 
    {
      computed[pos] = computed[pos - AES256_KEY_SIZE] ^ t[a];

      pos++;
    }
  }

  // If the computed schedule doesn't match our goal,
  // punt immediately!
  if (hamming_distance(in, computed, AES256_KEY_SCHEDULE_SIZE) > 8)
    return FALSE;

  return TRUE;
}

aes128_schedule* reconstruct_aes128(const unsigned char * in, aes128_schedule* schedule, int bits, int pos)
{
    if (bits == 0)
    {
        schedule->boolean = valid_aes128_schedule(in, schedule->schedule);
        return schedule;
    }
    schedule->schedule[pos/8] ^= (1 << (pos % 8));
    aes128_schedule *temp = malloc(sizeof(struct aes128_schedule));
    for (int i=pos+1; i < 8*AES128_KEY_SIZE; i++)
    {
        memcpy(temp, schedule, sizeof(struct aes128_schedule));
        temp->boolean = valid_aes128_schedule(in, schedule->schedule);
        if (temp->boolean == TRUE)
            return temp;
        temp = reconstruct_aes128(in, schedule, bits-1, i);
    }
}

aes192_schedule* reconstruct_aes192(const unsigned char * in, aes192_schedule* schedule, int bits, int pos)
{
    if (bits == 0)
    {
        schedule->boolean = valid_aes192_schedule(in, schedule->schedule);
        return schedule;
    }
    schedule->schedule[pos/8] ^= (1 << (pos % 8));
    aes192_schedule *temp = malloc(sizeof(struct aes192_schedule));
    for (int i=pos+1; i < 8*AES192_KEY_SIZE; i++)
    {
        memcpy(temp, schedule, sizeof(struct aes192_schedule));
        temp->boolean = valid_aes192_schedule(in, schedule->schedule);
        if (temp->boolean == TRUE)
            return temp;
        temp = reconstruct_aes192(in, schedule, bits-1, i);
    }
}

aes256_schedule* reconstruct_aes256(const unsigned char * in, aes256_schedule* schedule, int bits, int pos)
{
    if (bits == 0)
    {
        schedule->boolean = valid_aes256_schedule(in, schedule->schedule);
        return schedule;
    }
    schedule->schedule[pos/8] ^= (1 << (pos % 8));
    aes256_schedule *temp = malloc(sizeof(struct aes256_schedule));
    for (int i=pos+1; i < 8*AES256_KEY_SIZE; i++)
    {
        memcpy(temp, schedule, sizeof(struct aes256_schedule));
        temp->boolean = valid_aes256_schedule(in, schedule->schedule);
        if (temp->boolean == TRUE)
            return temp;
        temp = reconstruct_aes256(in, schedule, bits-1, i+1);
    }
}

